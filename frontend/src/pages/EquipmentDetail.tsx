...
export function EquipmentDetail() {
  ...
  if (!equipment) return <div className="text-center py-12 text-slate-500">Matériel introuvable</div>;
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {/* Photo */}
      <div className="h-56 sm:h-72 bg-slate-100 dark:bg-slate-800 rounded-xl overflow-hidden mb-6 flex items-center justify-center">
        {equipment.photos[0] ? (
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-6 mb-6">
        ...
        {equipment.description && (
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-4 whitespace-pre-line">{equipment.description}</p>
        )}
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100 dark:border-slate-800">
          <Link to={`/users/${equipment.owner.id}`} className="flex items-center gap-3 no-underline hover:underline">
            ...
            <div>
              <p className="text-sm font-medium text-slate-900 dark:text-slate-100">{equipment.owner.firstName} {equipment.owner.lastName}</p>
